  ...
  return (
    <Grid className='gap-6'>
        ...
        <Grid className='md:grid-cols-3 gap-5'>
            ...
            icon
            num={formatCurrency(data?.totalInflow)} 
            info={<TrendingDown size={40} className='text-[#05966849]'/>}
            others={<Flex className='gap-1.5 mt-1.5'>
                <Typography className='py-1 px-2.5 bg-accent text-chart-2 rounded-md' startIcon={<ArrowUp size={14}/>}>0%</Typography>
            ...
            icon
            num={formatCurrency(data?.totalOutflow)}  
            info={<TrendingUp size={40} className='text-destructive/40' />}
            others={<Flex className='gap-1.5 mt-1.5'>
                <Typography color='destructive' className='py-1 px-2.5 bg-accent rounded-md' startIcon={<ArrowUp size={14}/>}>0%</Typography>
            ...
            icon
            num={`₦${(data?.totalTransactionAmount).toLocaleString()}`} 
            info={<BookOpenText size={40} className='text-primary/40' />}
            others={<Typography variant='small' color='active'>Net Average: {formatCurrency(data?.averageTransactionAmount)}</Typography>}
            />
        </Grid>
    </Grid>
  )
